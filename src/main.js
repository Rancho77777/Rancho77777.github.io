...
const ui = {
  zh: {
    ...
    experience: '经历',
    identity: '身份',
    credentialTitle: '个人信息',
    credentialIntro: '这里有我的所有个人信息，包括身份、学籍、工作与社交媒体等内容。在这里你可以看见，在脱光衣服之前，我是一个什么样的人。',
    photoTitle: '照片<br><em>墙。</em>',
    photoIntro: '哪一个才是真实的我呢？那个天真、拘谨，认真学习努力工作的好女孩，还是那个淫荡、欲求不满、一丝不挂地张开双腿的母狗？',
    folderHelp: '或许我可以一直装成乖巧的绵羊，但我想要一次彻底的放纵，把我的一切都交给你随意羞辱践踏。',
    imagePending: '待放置图片',
    footerLabel: '尹伊果',
    footerTitle: '情欲、羞辱与\n湿漉漉的下体。',
  },
...
function photoFlow() {
  const files = person.media?.photoFiles || [];
  const rows = [0, 1, 2]
    .map(row => {
      const ordered = files.filter((_, index) => index % 3 === row);
      const items = [...ordered, ...ordered, ...ordered];
      return `<div class="photo-row row-${row + 1}">${items
        .map(
          (file, index) =>
            `<div class="flow-photo" style="--ratio:${[1.28, 0.82, 1.55, 1.05][(index + row) % 4]}"><img src="${assetPath(file)}" alt="${personName} photo ${index + 1}" onload="this.parentElement.style.setProperty('--ratio', this.naturalWidth / this.naturalHeight)" onerror="this.style.display='none'"></div>`
        )
        .join('')}</div>`;
    })
    .join('');
