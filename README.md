...
- 当前页面和数据文件均使用中文。
- 页面结构与顶部导航路由：编辑 `src/main.js`。当前导航页面为 `/`、`/about/`、`/info/`、`/photos/`、`/motion/`，各页面入口分别位于对应目录的 `index.html`。
...
- 尹伊果头像分别放在 `public/images/yin-yiguo/avatar/home/` 和 `public/images/yin-yiguo/avatar/overview/`：主页头像使用 `1.jpg` 到 `6.jpg`，移开鼠标恢复 `1.jpg`，每次重新 hover 按 `2.jpg` 到 `6.jpg` 顺序轮换；概览页仍使用 `1.*` 默认、`2.*` hover。具体路径在 `src/data.js` 的 `media.avatar` 中配置。
